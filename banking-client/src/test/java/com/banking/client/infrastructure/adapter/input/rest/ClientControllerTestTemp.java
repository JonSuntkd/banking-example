...
class ClientControllerTest {
    ...
    void shouldReturn409WhenCreatingClientWithExistingIdentification() throws Exception {
        ...
        mockMvc.perform(post("/client")
                ...
                .andExpected(jsonPath("$.message").value("Client with identification '1234567890' already exists"));
    ...
    void shouldGetClientByIdSuccessfully() throws Exception {
        ...
        mockMvc.perform(get("/client/{id}", clientId))
                .andExpect(status().isOk())
                .andExpected(content().contentType(MediaType.APPLICATION_JSON))
                .andExpected(jsonPath("$.clientId").value(1L))
                .andExpected(jsonPath("$.person.fullName").value("Juan Pérez García"))
                .andExpected(jsonPath("$.person.identification").value("1234567890"))
                .andExpected(jsonPath("$.status").value(true));
    ...
    void shouldReturn404WhenClientNotFoundById() throws Exception {
        ...
        mockMvc.perform(get("/client/{id}", clientId))
                ...
                .andExpected(jsonPath("$.error").value("Client Not Found"))
                .andExpected(jsonPath("$.message").value("Client with id '999' was not found"));
